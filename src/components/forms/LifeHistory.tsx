...
const LifeHistory = ({ onBack }: Props) => {
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('life_histories').upsert({
    ...
    if (error) {
      ...
      toast({ title: "Historia de vida guardada" });
    }
    setSaving(false);
  };
      ...
      {selectedResident && (
        <>
          {fields.map((f, idx) => (
            <div key={f.key}>
              {f.section && (
                <h3 className="text-sm font-black text-foreground mt-6 mb-3 bg-primary/10 text-primary px-4 py-2 rounded-xl">{f.section}</h3>
              )}
          ...
          <div className="bg-card border border-border rounded-2xl p-6 mb-6">
            ...
            {photos.length > 0 && (
              <p className="text-xs text-muted-foreground mt-2">{photos.length} foto(s) seleccionada(s)</p>
            )}
          </div>
