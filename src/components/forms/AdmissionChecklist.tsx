...
const AdmissionChecklist = ({ onBack }: Props) => {
  ...
  const handleSave = async () => {
    ...
    let error;
    if (existingId) {
      ({ error } = await supabase.from('admission_checklists').update(payload).eq('id', existingId));
    } else {
      ({ error } = await supabase.from('admission_checklists').insert(payload));
    }
    ...
    if (error) {
      ...
      await supabase.from('audit_log').insert({
        user_id: user.id,
        action: existingId ? 'update_admission_checklist' : 'create_admission_checklist',
        entity_type: 'admission_checklist',
        entity_id: selectedResident,
        ...
        <>
          ...
          <div className={`flex items-center gap-3 p-4 rounded-2xl mb-6 ${isComplete ? 'bg-cat-nutritional/10 text-cat-nutritional' : 'bg-cat-fragility/10 text-cat-fragility'}`}>
            ...
            <div className="flex-1 ml-4">
              <div className="h-2 bg-border rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${isComplete ? 'bg-cat-nutritional' : 'bg-cat-fragility'}`}
          ...
          <div className="bg-card border border-border rounded-2xl p-6 mb-6">
            ...
            <textarea
              ...
              placeholder="Notas adicionales sobre la documentación..."
            />
          </div>
