...
const NursingNotes = ({ onBack }: Props) => {
  ...
  const loadHistory = () => {
    ...
    if (!isConsolidated && selectedResident) {
      ...
      // Si no se ha seleccionado residente ni consolidada, no cargar nada
      setHistory([]);
      return;
    }
  ...
  const generateNote = async () => {
    ...
    try {
      ...
      if (data?.note) setNote(data.note);
        ...
        toast({
          title: "Sin datos",
          description: "No se encontraron registros para generar la nota",
          variant: "destructive",
        });
    } catch (e: any) {
  ...
  return (
    <div className="animate-fade-in">
      <FormHeader
        title="Notas de Enfermería con IA"
        subtitle="Generación automática de notas profesionales"
        onBack={onBack}
      />
        ...
        <div ref={contentRef} className="bg-card border border-border rounded-2xl p-6 mb-6">
          ...
          <div className="flex flex-wrap gap-3 mt-4">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-green-600 text-white px-6 py-3 rounded-xl text-xs font-bold hover:bg-green-700 transition"
            >
              <MessageCircle size={14} /> WhatsApp
            </button>
            <button
              onClick={shareEmail}
              className="flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-xl text-xs font-bold hover:bg-blue-700 transition"
            >
              <Mail size={14} /> Email
            </button>
            <button
              onClick={shareSocial}
              className="flex items-center gap-2 bg-purple-600 text-white px-6 py-3 rounded-xl text-xs font-bold hover:bg-purple-700 transition"
            >
                ...
                <div key={h.id} className="border border-border rounded-xl overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                      ...
                      {h.is_ai_generated && (
                        <span className="bg-accent text-accent-foreground px-2 py-0.5 rounded-md font-bold">IA</span>
                      )}
                      {h.is_consolidated && (
                        <span className="bg-secondary text-secondary-foreground px-2 py-0.5 rounded-md font-bold">
                          Consolidada
                        </span>
                      )}
                    </div>
                  ...
                  {!isExpanded && <div className="px-4 py-2 text-sm text-muted-foreground line-clamp-2">{h.note}</div>}
